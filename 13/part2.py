...
while True:
    if ball_position and paddle_position is not None:
        comp.set_both_inputs(np.sign(ball_position[0] - paddle_position[0]))
    ...
    if p == (-1, 0):
        ...
        if tile == BLOCK:
            if p not in block_tiles:
                block_tiles.append(p)
        else:
            if p in block_tiles:
                block_tiles.remove(p)
